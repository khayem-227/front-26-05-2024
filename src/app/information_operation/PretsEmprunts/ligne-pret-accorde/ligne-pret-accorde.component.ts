...
@Component({
  selector: 'app-ligne-pret-accorde',
  templateUrl: './ligne-pret-accorde.component.html',
  styleUrl: './ligne-pret-accorde.component.css'
})
export class LignePretAccordeComponent implements OnInit {
 ...
 accorde!:PretAccorde;
  ...
  submit(): void {
    if (this.form.valid) {
      ...
      this.serv.add(this.accorde).subscribe({
        next: (Response) => {
          console.log('Success:', Response);
          this.router.navigate(['/pret-contracte']);
        },
        error: (error) => {
